...
// https://www.codewars.com/kata/52c4dd683bfd3b434c000292/javascript

function isInteresting(number, awesomePhrases) {

    //number and the next two numbers
    number = [number, number + 1, number + 2];

    for(let i = 0; i < 3; i++) {

      const str = number[i].toString();

        //only numbers over 98
        if(number[i] < 98 && !awesomePhrases.includes(number[i])) {
            return 0;
        }
        ...
        //all the same digit
        if (/^(\d)\1+$/.test(str) && str.length > 2) {
            return i > 0 ? 1 : 2;
        }

        //incrementing or decrementing sequence
        if(("1234567890".includes(str) || "9876543210".includes(str)) && str.length > 2) {
            return i > 0 ? 1 : 2;
        }

        //palindrome
        if (str === str.split('').reverse().join('') && str.length > 2) {
            return i > 0 ? 1 : 2;
        }
